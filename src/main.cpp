      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          auto num_waypoints = ptsx.size();
          auto ptsx_vehicle = Eigen::VectorXd(num_waypoints);
          auto ptsy_vehicle = Eigen::VectorXd(num_waypoints);

         // Waypoints to vehicle coordinates
          for (unsigned int i = 0; i < num_waypoints; i++ ) {
            double delta_X = ptsx[i] - px;
            double delta_Y = ptsy[i] - py;
            ptsx_vehicle(i) = delta_X * cos( -psi ) - delta_Y * sin(-psi);
            ptsy_vehicle(i) = delta_X * sin( -psi ) + delta_Y * cos(-psi);
          }

          // Fit 3rd order polinomial
          ...
          double v_y = 0;
          double v_psi = 0;
          double v_cte = coeffs[0];
          double v_epsi = -atan(coeffs[1]);

          Eigen::VectorXd state(6);
          state << v_x, v_y, v_psi, v, v_cte, v_epsi;
